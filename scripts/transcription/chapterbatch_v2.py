...
from paths import INPUT_ICELANDIC, OUTPUT_ICELANDIC
...
def transcribe_all_chapters():
    ...
    print(f"🚀 Version 2 - Optimized with SDPA + Anti-Hallucination")

    # 2. Load AI (only do this once for all files)
    print("Loading AI model...")
    processor = AutoProcessor.from_pretrained(MODEL_ID)

    # ✨ OPTIMIZATION 1: Add SDPA attention implementation
    # This uses Apple Neural Engine optimization for ~20% speed boost
    model = AutoModelForSpeechSeq2Seq.from_pretrained(
        MODEL_ID,
        dtype=torch.float32,  # Required for MPS stability (avoid NaN)
        low_cpu_mem_usage=True,
        attn_implementation="sdpa"  # ← NEW: Scaled Dot Product Attention for Apple Silicon
    ).to(device)

    print("✓ Model loaded with SDPA attention optimization")
    ...
    for filename in audio_files:
        ...
        for chunk in tqdm(chunks, desc=f"Transcribing {filename}"):
            ...
            with torch.no_grad():
                # ✨ OPTIMIZATION 2: Add anti-hallucination parameters
                # Prevents repetition loops during silence or music
                predicted_ids = model.generate(
                    input_features,
                    language="icelandic",
                    task="transcribe",
                    repetition_penalty=1.1,            # ← NEW: Penalizes repeating n-grams
                    no_repeat_ngram_size=3             # ← NEW: Hard blocks 3-word loops
                )
        ...
        output_file_json = audio_path + ".v2.json"  # Different filename to avoid overwriting v1
        ...
        # Save derived formats with v2 suffix
        output_file_ts = audio_path + "_TRANSCRIPT.v2.txt"
        with open(output_file_ts, "w", encoding="utf-8") as f:
            f.write(formatted_timestamps_punct)

        output_file_md = audio_path + "_MARKDOWN.v2.md"
        with open(output_file_md, "w", encoding="utf-8") as f:
            f.write(formatted_markdown)

        output_file_llm = audio_path + "_LLM.v2.txt"
